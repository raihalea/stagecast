...
| **R12-followup-2** | S5    | SFU と Egress を同一 ECS Task に sidecar 同居 (ADR 0010)                     | `claude/r12-egress-sidecar` (**コード完**)         | Egress を SFU TaskDef の sidecar として配置し localhost で疎通 (`ws://localhost:7880`)。Valkey 維持。Task は 2 vCPU / 4 GiB に増強。独立 Egress Service を廃止。psrpc 登録が解決しない場合は ADR 0010 D-6 で Valkey 非Serverless に切替                                                                                                                                                                                                     |
