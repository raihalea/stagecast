...
| **R12-followup-14** | S5    | coturn sidecar + 静的認証 (ADR 0011 案 C)                                          | `claude/r12-coturn-sidecar` (PR 起票予定)                    | LiveKit 内蔵 TURN を `enabled: false` で停止し、 coturn を SFU TaskDef に sidecar 同居。 LiveKit yaml の `rtc.turn_servers` に `host=<NODE_IP>, port=3478, protocol=udp, username=stagecast, credential=<LIVEKIT_API_SECRET>` を静的設定 (entryPoint の sed で実値置換)。 coturn 側は `--lt-cred-mech --user=stagecast:$TURN_SECRET` で long-term credential 認証。 同じ Secret (LIVEKIT_API_SECRET) を LiveKit と coturn 両方で参照することで一致を保証 |
