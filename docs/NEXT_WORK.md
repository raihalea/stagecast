...
| **R12-followup-4** | S5    | SFU/Egress イメージを互換ペアにピン留め                                       |                                                    | LiveKit Egress リリースノートで v1.x.y と互換性のある livekit-server バージョンを特定し、両方を明示タグでピン留め。例えば `livekit/livekit-server:v1.9.0` + `livekit/egress:v1.8.4` 等。実機検証で psrpc topic 登録が成功するペアを探す |
